...
{#if isEditing}
	<div class="rounded-xs border border-border bg-muted p-4">
		<div class="mb-3 flex items-center justify-between gap-2">
			<p class="text-xs font-semibold tracking-wide text-muted-fg uppercase">Examples of skills</p>
...
{:else if skills.length > 0}
	<div class="flex-shrink-0 rounded-xs p-4">
		<p class="mb-3 text-xs font-semibold tracking-wide text-muted-fg uppercase">
			{language === 'sv' ? 'Exempel på färdigheter' : 'Examples of skills'}
		</p>
		<div class="flex flex-wrap gap-1">
			{#each skills as skill}
				<span class="rounded-xs bg-muted px-2 py-0.5 text-xs text-muted-fg">{skill}</span>
			{/each}
		</div>
	</div>
{/if}
